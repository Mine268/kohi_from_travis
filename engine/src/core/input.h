...
void input_shutdown();
void input_update(f64 delta_time); // 每帧调用一次

// 键盘状态查询接口
// 可以查询上一帧的状态，这样子可以感知按键变化
KAPI b8 input_is_key_down(keys key); // 当前帧是否按下
KAPI b8 input_is_key_up(keys key); // 当前帧是否释放
KAPI b8 input_was_key_down(keys key); // 上一帧是否按下
KAPI b8 input_was_key_up(keys key); // 上一帧是否释放

void input_process_key(keys key, b8 pressed); // 暴露给platform代码的，用来处理引擎的按键状态

// 鼠标
KAPI b8 input_is_button_down(buttons button); // 当前帧是否按下
KAPI b8 input_is_button_up(buttons button); // 当前帧是否释放
KAPI b8 input_was_button_down(buttons button); // 上一帧是否按下
KAPI b8 input_was_button_up(buttons button); // 上一帧是否释放
KAPI void input_get_mouse_position(i32* x, i32* y); // 获取鼠标位置
KAPI void input_get_previous_mouse_position(i32* x, i32* y);

void input_process_button(buttons button, b8 pressed); // 暴露给platform代码的，用来处理引擎的鼠标按键状态
